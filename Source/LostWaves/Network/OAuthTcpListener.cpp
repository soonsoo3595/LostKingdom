...
bool UOAuthTcpListener::Start(uint16 Port, UWorld* InWorldContext)
{
	...
	{
		UE_LOG(LogLostWaves, Error, TEXT("[OAuth] Failed to bind or listen on port %d"), Port);
		Stop();
		return false;
	}
	...
	UE_LOG(LogLostWaves, Log, TEXT("[OAuth] TCP Listener started on port %d"), Port);
	return true;
}
...
void UOAuthTcpListener::HandleClient(FSocket* ClientSocket)
{
	...
	{
		UE_LOG(LogLostWaves, Warning, TEXT("[OAuth] Client connection timed out waiting for data."));
		ClientSocket->Close();
	...
	if (ClientSocket->Recv(Buffer.GetData(), Buffer.Num(), BytesRead))
	{
		...
		UE_LOG(LogLostWaves, Log, TEXT("[OAuth] Received Request:\n%s"), *RequestStr);
		...
		if (RequestStr.Contains(TEXT("GET /oauth2callback")))
		{
			...
			if (!AuthCode.IsEmpty())
			{
				...
				AsyncTask(ENamedThreads::GameThread, [this, AuthCodeCopy]()
					{
						UE_LOG(LogLostWaves, Log, TEXT("[OAuth] Entered AsyncTask"));

						if (ULWAccountManager* AM = ULWAccountManager::Get(WorldContext))
						{
							UE_LOG(LogLostWaves, Log, TEXT("[OAuth] Calling RequestGoogleLoginWithAuthCode"));
							AM->RequestGoogleLoginWithAuthCode(AuthCodeCopy);
						}
						else
						{
							UE_LOG(LogLostWaves, Warning, TEXT("[OAuth] AccountManager is null"));
						}
					});
			}
			else
			{
				UE_LOG(LogLostWaves, Warning, TEXT("[OAuth] Failed to extract auth code."));
			}
		}
	}
